...
public class FlappySquare implements ActionListener{
	...
	public void addColumn(boolean a) {
		...
		if(a) {
		columns.add(new Rectangle(width + Width + columns.size()*125 , height - Height - 145, Width, Height));
		columns.add(new Rectangle(width + Width + (columns.size() - 1)*125,0, Width, height -Height - space ));
	}	
		else {
			columns.add(new Rectangle(columns.get(columns.size() - 1).x + 100, height - Height - 145, Width, Height));
